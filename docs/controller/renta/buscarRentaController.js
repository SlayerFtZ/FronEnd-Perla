document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!token || !id || !rol || !estado || estado.toLowerCase() === "inactivo") {
    console.log(
      "No se encontraron credenciales válidas. Redirigiendo al login."
    );
    localStorage.clear();
  ...
  function renderizarTablaRentas(rentas) {
    ...
    rentas.forEach((renta) => {
      const botonEstadoPago =
        renta.estadoPago === "Pagado"
      ...
      let botonInfo = "";
      if (
        (renta.estadoPago === "Abono" || renta.estadoPago === "Pagado") &&
        renta.abonos &&
        renta.abonos.length > 0
      ) {
        botonInfo = `
      ...
      switch (renta.estadoPago) {
        case "Pagado":
          ...
          break;
        case "Abono":
          claseFondoEstado = "bg-warning text-dark";
          break;
        case "Pendiente":
      ...
      fila.innerHTML = `
            ...
            <td>
                <img src="${
                  renta.fotoPerfilUrl || "../../images/perfilUsuario.jpg"
                }" width="80" class="rounded-circle me-2">
                ${renta.nombreUsuario} ${renta.apellidoPaterno} ${
        renta.apellidoMaterno
      }
            </td>
    ...
    document.querySelectorAll(".btn-abonar").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (inputAdeudo)
          inputAdeudo.value = `$${parseFloat(adeudo).toFixed(2)}`;
  ...
  if (formPago) {
    formPago.addEventListener("submit", function (e) {
      ...
      const montoRenta = parseFloat(
        document.getElementById("montoRenta").value
      );
      ...
      if (isNaN(adeudo) || montoRenta <= 0) {
        ...
        Swal.fire({
          title: "Procesando...",
          text: "Por favor espera mientras se realiza el abono.",
        ...
        fetch(
          `https://laperlacentrocomercial.dyndns.org/api/rentas/${idRenta}/abono?monto=${montoRenta}&&idUsuarioQuienRegistra=${idUsuarioRegistro}`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
